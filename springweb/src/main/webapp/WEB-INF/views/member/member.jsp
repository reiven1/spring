...
div#enroll-container input, div#enroll-container select {
	...
	<div id="enroll-container">
		<form name="memberEnrollFrm"
			action="${pageContext.request.contextPath }/member/insertend.do"
			method="post">
			<input type="text" class="form-control" placeholder="아이디 (4글자이상)"
				name="userId" id="userId_" required>
			<span id="id-result"></span>
			<input type="password" class="form-control" placeholder="비밀번호"
				name="password" id="password_" required> <input
				type="password" class="form-control" placeholder="비밀번호확인"
				id="password2" required> <input type="text"
				class="form-control" placeholder="이름" name="userName" id="userName"
				required> <input type="number" class="form-control"
				placeholder="나이" name="age" id="age"> <input type="email"
				class="form-control" placeholder="이메일" name="email" id="email"
				required> <input type="tel" class="form-control"
			...
			<div class="form-check-inline form-check">
				취미 : &nbsp; <input type="checkbox" class="form-check-input"
					...
					class="form-check-input" name="hobby" id="hobby1" value="등산">
				<label for="hobby1" class="form-check-label">등산</label>&nbsp; <input
				...
				<input type="checkbox" class="form-check-input" name="hobby"
					...
					class="form-check-input" name="hobby" id="hobby4" value="여행">
...
<script>
	document.getElementById("userId_")
	.addEventListener("keyup",((prev)=>{
		 return e=>{
					const userId=e.target.value.trim();
					if(userId.length>=4){
						if(prev) clearTimeout(prev);
						prev=setTimeout(async ()=>{
								const response=await fetch("${pageContext.request.contextPath}/member/idduplicate.do?userId="+userId);
								const data=await response.text();
								let msg="사용할 수 있습니다",color="green";
								if(data=='false'){
									msg="사용할 수 없습니다";
									color="red";
								}
								document.getElementById("id-result").style.color=color;
								document.getElementById("id-result").innerText=msg;
							},500)
					}//if문 닫기
		 		}//이벤트핸들러 닫기
		})()//이벤트핸들러를 반환하는 함수 자동실행
	)//addEventListener닫기
</script>
